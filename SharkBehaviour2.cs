...
public class SharkBehaviour2 : MonoBehaviour
{
	...
	{
		float strength = m_Strength * Mathf.Sign(Random.Range(-1, 1));
		for (int i = 0; (float)i < duration; i++)
		{
			...
			foreach (Rigidbody rigidbody in tail)
			{
				...
				rigidbody.AddRelativeTorque(dir * strength, ForceMode.Impulse);
			}
			yield return new WaitForFixedUpdate();
		}
		strength *= -0.5f;
		for (int k = 0; (float)k < duration; k++)
		{
			Rigidbody[] tail2 = m_Tail;
			foreach (Rigidbody rigidbody2 in tail2)
			{
				base.transform.InverseTransformDirection(dir);
				rigidbody2.AddRelativeTorque(dir * strength, ForceMode.Impulse);
			}
			yield return new WaitForFixedUpdate();
		}
		yield return null;
