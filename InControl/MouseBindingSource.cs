...
public class MouseBindingSource : BindingSource
{
	...
	internal static float GetValue(Mouse mouseControl)
	{
		...
		{
			Mouse.NegativeX => 0f - Mathf.Min(SingletonMonoBehavior<InControlManager, MonoBehaviour>.Instance.mouseValue.x * ScaleX, 0f), 
			Mouse.PositiveX => Mathf.Max(0f, SingletonMonoBehavior<InControlManager, MonoBehaviour>.Instance.mouseValue.x * ScaleX), 
			Mouse.NegativeY => 0f - Mathf.Min(SingletonMonoBehavior<InControlManager, MonoBehaviour>.Instance.mouseValue.y * ScaleY, 0f), 
			Mouse.PositiveY => Mathf.Max(0f, SingletonMonoBehavior<InControlManager, MonoBehaviour>.Instance.mouseValue.y * ScaleY), 
			Mouse.NegativeScrollWheel => 0f - Mathf.Min(SingletonMonoBehavior<InControlManager, MonoBehaviour>.Instance.mouseValue.z * ScaleZ, 0f), 
			Mouse.PositiveScrollWheel => Mathf.Max(0f, SingletonMonoBehavior<InControlManager, MonoBehaviour>.Instance.mouseValue.z * ScaleZ), 
			_ => 0f, 
		};
	}
