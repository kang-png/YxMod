...
public class Pushing : MonoBehaviour
{
	...
	public void FixedUpdate()
	{
		if (negativeForce < 0f - tresholdLow && positiveForce > tresholdLow)
		{
			...
			{
				float num = Mathf.InverseLerp(2f, 0f, negativeBody.velocity.magnitude);
				negativeBody.AddForceAtPosition(num * direction * Mathf.Clamp(negativeForce * multiplier, 0f - maxForce, 0f), negativePoint, ForceMode.Force);
			}
			if (positiveBody != null && !positiveBody.isKinematic)
			{
				float num2 = Mathf.InverseLerp(2f, 0f, positiveBody.velocity.magnitude);
				positiveBody.AddForceAtPosition(num2 * direction * Mathf.Clamp(positiveForce * multiplier, 0f, maxForce), positivePoint, ForceMode.Force);
			}
		}
