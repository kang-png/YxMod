			...
			if (m_textInfo.meshInfo[j].vertices == null || m_textInfo.meshInfo[j].vertices.Length < referenceCount * 4)
			{
				if (m_textInfo.meshInfo[j].vertices == null)
				{
					...
					else
					{
						...
						reference2 = new TMP_MeshInfo(m_subTextObjects[j].mesh, referenceCount + 1);
					}
				}
				else
				{
			...
			case TextAlignmentOptions.CaplineFlush:
			{
				...
				if ((!char.IsControl(character3) && lineNumber < m_lineNumber) || flag13 || tMP_LineInfo.maxAdvance > tMP_LineInfo.width)
				{
					...
					if (character2 == '\t' || char.IsSeparator(character2))
					{
						if (!m_isRightToLeft)
						{
							vector11 += new Vector3(num50 * (1f - num53) / (float)num52, 0f, 0f);
						}
						else
						{
							vector11 -= new Vector3(num50 * (1f - num53) / (float)num52, 0f, 0f);
					...
					{
						vector11 += new Vector3(num50 * num53 / (float)num51, 0f, 0f);
					}
					else
					{
						vector11 -= new Vector3(num50 * num53 / (float)num51, 0f, 0f);
			...
			if (isVisible)
			{
				...
				case TMP_TextElementType.Character:
				{
					...
					case TextureMappingOptions.Line:
						...
						{
							characterInfo[j].vertex_BL.uv2.x = (characterInfo[j].vertex_BL.position.x + vector11.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num54;
							characterInfo[j].vertex_TL.uv2.x = (characterInfo[j].vertex_TL.position.x + vector11.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num54;
							characterInfo[j].vertex_TR.uv2.x = (characterInfo[j].vertex_TR.position.x + vector11.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num54;
							characterInfo[j].vertex_BR.uv2.x = (characterInfo[j].vertex_BR.position.x + vector11.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num54;
						}
						break;
