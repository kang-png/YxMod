...
public class CustomizationCameraController : MonoBehaviour
{
	...
	private void Update()
	{
		...
		float num = cameraArm.rotation.eulerAngles.x;
		float num2 = cameraArm.rotation.eulerAngles.y;
		float num3 = cameraHolder.transform.localPosition.x;
		float num4 = cameraHolder.transform.localPosition.y;
		float num5 = 0f - cameraHolder.transform.localPosition.z;
		if (num > 180f)
		{
			num -= 360f;
		}
		...
		{
			float num10 = 2f;
			num -= activeDevice.RightStick.Vector.y * num10;
			num = Mathf.Clamp(num, -89f, 89f);
			num2 += activeDevice.RightStick.Vector.x * num10;
			if (num2 > 180f)
			{
				num2 -= 360f;
			}
		...
		float num11 = activeDevice.LeftTrigger.Value - activeDevice.RightTrigger.Value;
		if (num11 != 0f)
		{
			float num12 = -0.2f * num11;
			num5 *= Mathf.Pow(1.1f, 0f - num12);
			num5 = Mathf.Clamp(num5, 0.2f, 5f);
		}
