...
public class NetTransportUNet : NetTransport
{
	...
	public override void JoinGame(object serverAddress, OnJoinGameDelegate callback)
	{
		...
		serverConnectionId = NetworkTransport.Connect(hostId, (string)serverAddress, 8888, 0, out var error);
		if (error != 0)
		{
	...
	{
		NetworkTransport.Send(hostId, GetConnectionID(host), reliableChannelId, data, len, out var error);
		if (error != 0)
		{
			NetworkError networkError = (NetworkError)error;
	...
	{
		NetworkTransport.Send(hostId, GetConnectionID(host), unreliableChannelId, data, len, out var error);
		if (error != 0)
		{
			NetworkError networkError = (NetworkError)error;
			Debug.LogError("Error: " + networkError);
	...
	public override void OnUpdate()
	{
		...
		do
		{
			...
			networkEventType = NetworkTransport.Receive(out var _, out var connectionId, out var _, array, array.Length, out var receivedSize, out var _);
