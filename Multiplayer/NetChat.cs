...
public class NetChat : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        //回车符聊天
        else if (Input.GetKeyDown(KeyCode.Return) && !typing && UI_SheZhi.huicheliaotian)
        {
            if (Options.parental == 0)
            {
                Show(showMessages: true, showType: true);
	...
	public static void OnReceive(uint clientId, string nick, string msg)
	{
		...
		if (netHost != null && !netHost.mute)
		{
			msg = msg.Replace('<', '〈');
			msg = msg.Replace('>', '〉');
			Print($"<#{HexConverter.ColorToHex(instance.colors[clientId % instance.colors.Length])}>{nick}</color> {msg}");
		}
	}
