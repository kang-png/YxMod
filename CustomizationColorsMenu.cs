...
public class CustomizationColorsMenu : MenuTransition
{
	...
	private void AddColorButtons()
	{
		...
		{
			ref Color reference = ref colors[i + 0 * baseColors.Length];
			reference = baseColors[i].Desaturate(0.2f).Desaturate(0.4f).Brighten((i != 0) ? 0.4f : 1f);
			ref Color reference2 = ref colors[i + baseColors.Length];
			reference2 = baseColors[i].Desaturate(0.2f).Desaturate(0.2f).Brighten(0.2f);
			ref Color reference3 = ref colors[i + 2 * baseColors.Length];
			reference3 = baseColors[i].Desaturate(0.2f);
			ref Color reference4 = ref colors[i + 3 * baseColors.Length];
			reference4 = baseColors[i].Desaturate(0.2f).Desaturate(0.2f).Darken(0.2f);
			ref Color reference5 = ref colors[i + 4 * baseColors.Length];
			reference5 = baseColors[i].Desaturate(0.2f).Desaturate(0.4f).Darken((i != 0) ? 0.4f : 1f);
		}
