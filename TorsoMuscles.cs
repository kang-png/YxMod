...
public class TorsoMuscles
{
	...
	{
		float num = 3f;
		Vector3 vector = bottom.position + bottom.velocity * Time.fixedDeltaTime - top.position - top.velocity * Time.fixedDeltaTime * num;
		vector.y = 0f;
		Vector3 vector2 = vector * top.mass / Time.fixedDeltaTime;
		vector2 *= multiplier;
		top.SafeAddForce(vector2);
		bottom.SafeAddForce(-vector2);
	}
}
