...
public sealed class LevelInformationBox : MonoBehaviour
{
    ...
    private IEnumerator GetNewLevel2(ulong levelID)
    {
        bool loaded = false;
    ...
    private void UpdateLevelUI(WorkshopLevelMetadata levelData)
    {
        LevelText.text = levelData.title;
        LevelImage.texture = levelData.thumbnailTexture;
        LevelImage.enabled = LevelImage.texture != null;
    }
    private IEnumerator GetNewLevel(ulong levelID) //关闭大厅下载   修改
    {
        ...
        WorkshopRepository.instance.levelRepo.LoadLevel(levelID, onRead);
        while (!loaded)
        {
            yield return null;
        }
