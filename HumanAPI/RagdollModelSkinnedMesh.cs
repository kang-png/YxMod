...
public class RagdollModelSkinnedMesh : MonoBehaviour
{
	...
	internal void Reskin2(Ragdoll ragdoll)
	{
		...
		{
			ref Matrix4x4 reference = ref bindposes[i];
			reference = RagdollTemplate.instance.bindposes[i] * localToWorldMatrix;
		}
		Mesh mesh = null;
		...
		for (int j = 0; j < vertices.Length; j++)
		{
			...
			reference2 = localToWorldMatrix2.MultiplyPoint3x4(vertices[j]);
		}
		BoneWeight[] array = new BoneWeight[vertices.Length];
		for (int k = 0; k < array.Length; k++)
		{
			ref BoneWeight reference3 = ref array[k];
			reference3 = RagdollTemplate.instance.Map(vertices[k]);
		}
	...
	private void Override(Ragdoll ragdoll, Vector3[] targetPositions, BoneWeight[] targetWeights, ref Transform[] bones, ref Matrix4x4[] bindposes, Matrix4x4 targetMeshToWorld)
	{
		...
		for (int i = 0; i < componentsInChildren.Length; i++)
		{
			...
			ref Matrix4x4 reference = ref bindposes[num + i];
			reference = skinOverrideVolume.GetBindPose(ragdoll.transform) * targetMeshToWorld;
		}
		...
		for (int j = 0; j < targetPositions.Length; j++)
		{
			...
			if (num2 != 0f)
			{
				...
				b.boneIndex2 += num;
				b.boneIndex3 += num;
				ref BoneWeight reference2 = ref targetWeights[j];
				reference2 = BoneWeightUtils.Lerp(targetWeights[j], b, num2);
			}
		}
	}
	...
	public void Clip(RigClipVolume[] clipVolumes)
	{
		...
		for (int i = 0; i < vertices.Length; i++)
		{
			...
			reference = reskinnedRenderer.transform.TransformPoint(vertices[i]);
		}
