...
public class RopeRender : MonoBehaviour
{
	...
	public virtual void OnEnable()
	{
		...
		{
			ref Vector2 reference = ref rotatedRadius[m];
			reference = new Vector2(radius, 0f).Rotate((float)Math.PI * 2f * (float)m / (float)segmentsAround);
		}
	}
	...
	public virtual void LateUpdate()
	{
		...
		for (int j = 0; j < meshSegments - num; j++)
		{
			...
			{
				ref Vector3 reference = ref meshVerts[idx];
				reference = meshVerts[idx - segmentsAround];
				idx++;
			}
		}
