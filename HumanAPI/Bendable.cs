	...
	public void ReportBend(int index, Vector3 maxImpactVector)
	{
		...
		for (int i = 0; i < bones.Length; i++)
		{
			...
			if (!(num2 <= 0f) && !(num <= 0f))
			{
				...
				Vector3 vector4 = (component.position = originalPositions[i] + Vector3.ClampMagnitude(vector2 - originalPositions[i], num * 0.5f));
				if ((vector4 - originalPositions[i]).magnitude > 0.1f)
				{
					isBent = true;
				}
			}
