...
public sealed class PostProcessLayer : MonoBehaviour
{
	...
	private void BuildCommandBuffers()
	{
		...
		if (num > 0)
		{
			...
			currentContext.GetScreenSpaceTemporaryRT(commandBuffer, num2, 0, renderTextureFormat);
			commandBuffer.BuiltinBlit(renderTargetIdentifier, num2, RuntimeUtilities.copyStdMaterial, stopNaNPropagation ? 1 : 0);
			currentContext.source = num2;
			int num3 = -1;
			if (num > 1)
			{
				num3 = m_TargetPool.Get();
				currentContext.GetScreenSpaceTemporaryRT(commandBuffer, num3, 0, renderTextureFormat);
		...
		currentContext.GetScreenSpaceTemporaryRT(m_LegacyCmdBuffer, num4, 0, renderTextureFormat, RenderTextureReadWrite.sRGB);
		m_LegacyCmdBuffer.BuiltinBlit(renderTargetIdentifier, num4, RuntimeUtilities.copyStdMaterial, stopNaNPropagation ? 1 : 0);
		if (!m_NaNKilled)
		{
