...
public class NameTag : MonoBehaviour
{
	...
	private int CalculateStringWidth()
	{
		...
		{
			textMesh.font.GetCharacterInfo(text[i], out var info, textMesh.font.fontSize);
			num += info.advance;
		}
		return num;
	}
	...
	private void BuildMesh(int stringWidth, int stringHeight)
	{
		...
		reference = new Vector3(0f - num, num2, kInitialZ);
		ref Vector3 reference2 = ref vertices[1];
		reference2 = new Vector3(num, num2, kInitialZ);
		ref Vector3 reference3 = ref vertices[2];
		reference3 = new Vector3(0f - num, 0f - num2, kInitialZ);
		ref Vector3 reference4 = ref vertices[3];
		reference4 = new Vector3(num, 0f - num2, kInitialZ);
	}
    ...
    private void Update()
    {
        if (UI_SheZhi.xianshimingzi)/////ÐÞ¸Ä
        {
            if (ShouldDisplay())
            {
                forceShow = true;
