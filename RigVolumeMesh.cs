...
public class RigVolumeMesh : MonoBehaviour
{
	...
	public void Build(float normalSign)
	{
		...
		for (int i = 0; i < triangles.Length; i += 3)
		{
			...
			{
				if (Mathf.Abs((faces[j].n - vector).magnitude) < 0.001f && Mathf.Abs(faces[j].d - num) < 0.001f)
				{
					flag = true;
					break;
				}
	...
	public float GetDistOutside(Vector3 pos)
	{
		...
		Vector3 vector = pos - face2.n * num2;
		for (int j = 0; j < faces.Count; j++)
		{
			if (j != num)
			{
				...
				if (!(num4 <= 0f))
				{
					Vector3 vector2 = face3.n - face2.n * Vector3.Dot(face3.n, face2.n);
					float num5 = Vector3.Dot(vector2, face3.n);
					vector += vector2 * num4 / num5;
				}
			}
		}
		return (pos - vector).magnitude;
	}
}
