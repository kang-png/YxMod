...
public class CameraController3 : MonoBehaviour
{
	...
	public void LateUpdate()
	{
		...
		if (num2 < offset && !Physics.SphereCast(vector2 - vector * offset, num * 2f, vector, out var _, offset - num2, wallLayers, QueryTriggerInteraction.Ignore))
		{
			offset = num2;
			offsetSpeed = 0f;
		}
	...
	private float CompensateForWallsNearPlane(Vector3 targetPos, Quaternion lookRot, float desiredDist, float minDist)
	{
		...
		ref Vector3 reference = ref rayStarts[0];
		reference = gameCam.ViewportToWorldPoint(new Vector3(0f, 0f, nearClipPlane));
		ref Vector3 reference2 = ref rayStarts[1];
		reference2 = gameCam.ViewportToWorldPoint(new Vector3(0f, 1f, nearClipPlane));
		ref Vector3 reference3 = ref rayStarts[2];
		reference3 = gameCam.ViewportToWorldPoint(new Vector3(1f, 0f, nearClipPlane));
		ref Vector3 reference4 = ref rayStarts[3];
		reference4 = gameCam.ViewportToWorldPoint(new Vector3(1f, 1f, nearClipPlane));
		float num = desiredDist - nearClipPlane;
		for (int i = 0; i < rayStarts.Length; i++)
		{
			Ray ray = new Ray(rayStarts[i], -vector);
			if (Physics.Raycast(ray, out var hitInfo, num, wallLayers) && hitInfo.distance < num)
			{
